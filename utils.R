...
# Projet : Baromètre de la Parité en Entreprise
...
#' Importe la dernière version du jeu de données Egapro depuis data.gouv.fr
#'
#' @description Recherche le dataset sur data.gouv, identifie la ressource
#' la plus récente (JSON, CSV, ou XLSX) et la télécharge.
#' @return Un dataframe contenant les données Egapro brutes.
...
#' Télécharge les données SIRENE pour l'IDF par tranches d'effectifs.
#'
#' @description Boucle sur une liste d'URLs pour récupérer les données SIRENE
#' des entreprises de +50 salariés en Île-de-France.
#' @return Un dataframe contenant les données SIRENE brutes.
...
#' Importe un fichier Excel contenu dans une archive ZIP locale.
#'
#' @param zip_path Chemin vers le fichier .zip local.
#' @param file_pattern Pattern pour trouver le fichier dans le zip. Par défaut "\\.xlsx$".
#' @param skip Nombre de lignes à sauter au début du fichier Excel.
#' @return Un dataframe avec les données du fichier Excel.
...
#' Crée la colonne des secteurs d'activité à partir du code NAF.
#' @param code_naf Vecteur de codes NAF.
...
#' Prépare et enrichit les données Egapro.
#' @param raw_egapro_df Le dataframe brut issu de `import_latest_egapro`.
#' @return Un dataframe Egapro propre et enrichi.
prepare_egapro_data <- function(raw_egapro_df) {
  ...
  if (length(cols_aug_existantes) > 0) {
    df_prepared <- df_prepared %>% mutate(note_augmentation_unifiee = coalesce(!!!rlang::syms(cols_aug_existantes)))
  } else {
    df_prepared$note_augmentation_unifiee <- NA_real_
  }
  
  df_prepared <- df_prepared %>%
    filter(structure == "Entreprise", !is.na(note_index)) %>%
    rename(
      index = note_index, tranche_effectifs = all_of(effectifs_col), code_naf = all_of(naf_col), 
      note_remuneration = note_ecart_remuneration, note_augmentation = any_of("note_augmentation_unifiee"),
    ...
    mutate(secteur_activite = get_secteur_from_naf(code_naf)) %>%
    select(siren, annee, index, raison_sociale, tranche_effectifs, code_naf, secteur_activite,
           any_of(c("note_remuneration", "note_augmentation", "note_promotion", "note_conge_mat", "note_hautes_rem"))) %>%
    distinct(siren, annee, .keep_all = TRUE)
...
#' Nettoie les données SIRENE pour ne garder que les sièges sociaux géolocalisés.
#' @param raw_sirene_df Le dataframe brut issu de `import_sirene_idf`.
#' @return Un dataframe avec une ligne par siège social (SIREN) et ses coordonnées.
clean_sirene_data <- function(raw_sirene_df) {
  message("--- Nettoyage des données SIRENE ---")
  sirene_clean <- raw_sirene_df %>%
    janitor::clean_names() %>%
    filter(etablissementsiege == "oui") %>%
    select(siren, code_commune = codecommuneetablissement, geolocetablissement) %>%
...
#' @return Un objet sf contenant les polygones des communes du périmètre.
load_and_prepare_map <- function() {
  ...
  map <- sf::st_read(myURL, quiet = TRUE) %>%
    ...
    mutate(across(c(com_arm_code, com_arm_name, ept_code, ept_name, dep_code, dep_name), as.character)) %>%
    select(com_code = com_arm_code, com_name = com_arm_name, ept_code, ept_name, dep_code, dep_name)
...
#' Agrège un fond de carte communal à un niveau supérieur (EPT ou DEP).
#' @param map_com_sf L'objet sf des communes.
#' @param level Le niveau d'agrégation : "ept" ou "dep".
#' @return Un objet sf agrégé.
...
#' Génère le contenu HTML pour le pop-up d'une entreprise sur la carte.
#' @param df_row Une ligne de dataframe contenant les infos de l'entreprise.
#' @return Un objet HTML pour le popup Leaflet.
...
#' Crée un switchInput pour la palette de couleurs accessible.
#' @param id L'identifiant unique pour le switchInput.
#' @return Un objet UI Shiny.
color_switch_ui <- function(id) {
  div(style = "text-align: right; margin-top: 15px;",
      switchInput(inputId = id, label = "Palette accessible", onLabel = "Oui", offLabel = "Non", value = FALSE,
